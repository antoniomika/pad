...
class PADBot {
  ...
  constructor(
    ...
    discordBotOwnerTag: string,
    ffmpegInput: string,
    registerExit: boolean = true,
    commandFlag: string = "",
    autoStartPCM: boolean = true,
  ) {
  ...
  async handleHelp(message: Message, handler: Handler): PromiseResult {
    const fields = [
      {
        name: "Command",
        value: Array.from(this.handlers.keys()).join("\n"),
        inline: true,
      },
      {
        name: "Help",
        value: Array.from(this.handlers.values())
          .map((h) => h.help)
          .join("\n"),
        inline: true,
      },
    ...
    if (cmdAndArgs.length > 1 && cmdAndArgs[1].includes("group")) {
      fields[2] = {
        name: "Permitted Groups",
        value: Array.from(this.handlers.values())
          .map((h) => h.permittedGroups.join(", "))
          .join("\n"),
        inline: true,
      };
    }
  ...
  async handleCommand(message: Message): PromiseResult {
    ...
    if (
      message.guild !== null &&
      this.state.guilds[message.guild.id] === undefined
    ) {
          ...
          handlerResult = await (async () => {
            for (const group in groups) {
              for (const member of groups[group]) {
                if (
                  member === message.member?.user?.tag &&
                  handler.permittedGroups.includes(group)
                ) {
      ...
      embeds: [
        {
          ...
          fields: [
            {
              name: "Group",
              value: Object.keys(groups).join("\n"),
              inline: true,
            },
            {
              name: "Members",
              value: Object.values(groups)
                // @ts-expect-error Members can be anything
                .map((g) => g.join(", "))
                .join("\n"),
              inline: true,
            },
    ...
    try {
      if (change.startsWith("-") || change.startsWith("+")) {
        if (change.endsWith("%")) {
          ...
          const effect =
            100 + (change.startsWith("-") ? -magnitude : magnitude);
          ...
          if (magnitude < 1) {
            const effect =
              1 + (change.startsWith("-") ? -magnitude : magnitude);
  ...
  async handleJoinURL(message: Message, handler: Handler): PromiseResult {
    ...
    return await message.channel.send(
      `https://discord.com/api/oauth2/authorize?client_id=${this.discordClientID}&permissions=103827520&scope=bot`,
    );
  }
